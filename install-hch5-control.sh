...
# Existing installations are NEVER sent through the first-install path. That
# path rewrites environment/config scaffolding and historically could make a
# recovery unnecessarily risky. Existing systems use the transactional updater,
# which preflights everything while the old process is still running and rolls
# back automatically if live validation fails.
...
# First installation only. Reuse a previously saved stable adapter path when
# recovering from a partial install. Otherwise auto-select only when exactly one
# stable /dev/serial/by-id adapter exists; never guess between multiple devices.
has_device=0
