...
# Nothing in the live installation has been changed yet. Any failure here aborts
# while the currently running controller keeps serving traffic.
...
# Validate authentication state before touching live code. If an owner exists,
# an update is never allowed to silently turn the installation into first-run.
...
# Only now create a rollback snapshot and touch live code.
...
# Second validation from the exact live paths. Still no service restart yet, so
# a bad copy can be rolled back without interrupting the old Python process.
...
# This is the first and only planned interruption.
...
# Auth API must be valid JSON and must still report configured when an owner
...
# Verify the controller API itself, not just the static WebUI.
...
# Verify the raw Home Assistant TCP endpoint is actually accepting connections.
python3 - "${raw_port}" <<'PY'
import socket, sys
port = int(sys.argv[1])
...
# Persistent owner/config files must still be untouched after service start.
