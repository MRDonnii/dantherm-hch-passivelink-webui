...
// Air colours follow temperature on one scale, so the drawing shows what a
// counterflow exchanger does: cold outdoor air picks up heat across the core
// while warm extract air gives it away, and the afterheat warms supply further.
...
const WATER_SUPPLY_TO: Point = [-40, 456];
const WATER_RETURN_TO: Point = [-40, 482];
const WATER_VALVE_AT: Point = [20, 482];
...
function Rs485Wiring({ active, valveAt }: { active: boolean; valveAt: Point }) {
  ...
  return <g className={`hch-wiring${active ? " active" : ""}`}>
    ...
    <path className="hch-signal-wire" d={`M${x} ${y + 36} H${valveAt[0]} V${valveAt[1]}`}/>
    ...
    <g className="hch-device hch-hac1-box"><rect className="device-body" x={x} y={y} width={width} height={height} rx="10"/><text x={cx} y={y + 22} textAnchor="middle">HAC1 styring</text><text className="device-sub" x={cx} y={y + 41} textAnchor="middle">Modbus-slave 0x40</text></g>
    <g className="hch-device hch-pi">
      ...
      <text x="710" y="506" textAnchor="middle">Raspberry Pi</text><text className="device-sub" x="710" y="525" textAnchor="middle">Gateway · RS485</text>
    </g>
  </g>;
}
...
export function Hch5UnitDiagram(props:Hch5UnitDiagramProps) {
  ...
  return <div className={`hch5-visual${bypassOpen?" is-bypass":" is-recovery"}`}>
    <svg viewBox={`${VIEW.x} ${VIEW.y} ${VIEW.width} ${VIEW.height}`} role="group" aria-label="HCH5 luftstrøm med intern bypass og ekstern eftervarme">
      ...
      <Rs485Wiring active={busActive} valveAt={water ? [WATER_VALVE_AT[0], WATER_VALVE_AT[1] + 14] : [23, 446]}/>
