...
// Oriented like the real HCH5: outdoor air (T1) and exhaust (T4) connect on
// the right, where both fan motors sit; extract (T3) and supply connect on
// the left, where the supply duct feeds the external HAC1 afterheat coil.
// Duct centrelines all meet the exchanger's exact rotation centre (562 295),
// crossing it as a clean X — the visual equivalent of real cross-flow. Both
// ends of each duct extend past the cabinet so the temperature ports can sit
// centred on the flow line itself (fog running through them) with a short,
// fading tail on either side instead of a hard cutoff.
const NORMAL_SUPPLY = "M1247 205 H784 Q762 205 743 222 L562 295 L383 350 Q362 365 336 365 H-238";
const NORMAL_EXTRACT = "M-238 205 H336 Q362 205 382 222 L562 295 L742 350 Q762 365 787 365 H1247";
// Only supply bypasses the core; extract still always crosses the exchanger.
const BYPASS_SUPPLY = "M1247 205 H786 Q756 205 756 178 V143 Q756 118 730 118 H398 Q368 118 368 145 V326 Q368 365 330 365 H-238";
...
function Rs485Wiring({ active }: { active: boolean }) {
  // Leaves the control box beside, not through, its "Elektronik" label.
  const bus = "M378 382 V496 M119 496 H600";
  return <g className={`hch-wiring${active ? " active" : ""}`}>
    ...
    <rect className="hch-cable-gland" x="370" y="428" width="16" height="12" rx="3"/>
    <circle className="hch-bus-joint" cx="378" cy="496" r="4"/>
    <text className="hch-bus-label" x="489" y="487" textAnchor="middle">RS485 · Modbus RTU</text>
  ...
  return <div className={`hch5-visual${bypassActual?" is-bypass":" is-recovery"}`}>
    <svg viewBox="-278 -8 1550 590" role="img" aria-label="HCH5 luftstrøm med intern bypass og ekstern eftervarme">
      ...
      <g filter="url(#unitShadow)">
        ...
        <path className="hch-frame" d="M261 151V414M416 151V414M702 151V414M855 151V414M211 283H903"/>
        <Filter x={267} y={207} label="Filter · udsugning"/><Filter x={852} y={207} label="Filter · udeluft"/>
        <Fan x={770} y={210} rpm={supplyRpm} label="Tilluft"/><Fan x={770} y={343} rpm={extractRpm} label="Fraluft"/>
        <g className={`hch-exchanger${bypassActual?" bypassed":""}`} transform="translate(562 295) rotate(-45)"><rect x="-91" y="-91" width="182" height="182" rx="13" fill="url(#exchangerMetal)"/>{[-60,-40,-20,0,20,40,60].map(o=><path key={o} d={`M-69 ${o}H69`}/>)}</g>
        <text className="hch-exchanger-title" x="562" y="291" textAnchor="middle">Varmeveksler</text><text className="hch-recovery" x="562" y="317" textAnchor="middle">{bypassActual?"BYPASS":recovery===null?"—":`${recovery}%`}</text>
        <g className={`hch-bypass ${bypassActual?"open":"closed"}`} transform="translate(562 145)"><rect x="-72" y="-17" width="144" height="34" rx="8"/><path className="bypass-rail" d="M-48 0H48"/><rect className="bypass-blade" x="-30" y="-5" width="60" height="10" rx="4" transform={bypassActual?"rotate(0)":"rotate(-62)"}/><text x="0" y="-28" textAnchor="middle">Bypass · {bypassActual?"åben":"lukket"}</text></g>
        <rect className="hch-service-box" x="289" y="310" width="100" height="72" rx="9"/><path className="service-lines" d="M302 325H374M302 339H361M302 353H370M302 367H351"/><text className="hch-part-label" x="339" y="400" textAnchor="middle">Elektronik</text>
      </g>
      ...
      <g className="hch-bypass-callout" transform="translate(479 20)"><rect width="166" height="53" rx="12"/><text x="83" y="20" textAnchor="middle">Bypass-spjæld</text><text className="bypass-state" x="83" y="40" textAnchor="middle">{bypassActual?"Åben":"Lukket"} · ønske {bypassRequest.toLowerCase()==="on"?"On":"Auto"}</text></g>
    </svg>
