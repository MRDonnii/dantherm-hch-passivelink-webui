...
describe("Hch5UnitDiagram",()=>{
  ...
  it("reroutes only supply in bypass and keeps the external coil",()=>{const m=renderToStaticMarkup(<Hch5UnitDiagram {...baseProps} bypassActual/>);expect(m).toContain("Q356 205 356 178 V143");expect(m).toContain("M1350 205 H776");expect(m).toContain("Ekstern eftervarme · HAC1");expect(m).toContain("is-bypass");});
  it("centres temperature ports on the duct flow line so fog runs through them",()=>{const m=renderToStaticMarkup(<Hch5UnitDiagram {...baseProps}/>);expect(m).toContain("hch-temp-port");expect(m).toContain('translate(-90 205)');expect(m).toContain('translate(-90 365)');expect(m).toContain('translate(1305 205)');expect(m).toContain('translate(1305 365)');expect(m).toContain("Udeluft");expect(m).toContain("Indblæsning");});
  it("fades the fog out with a mask instead of a hard cutoff", ()=>{const m=renderToStaticMarkup(<Hch5UnitDiagram {...baseProps}/>);expect(m).toContain("fogFadeMask");expect(m).toContain("fogFadeGradient");});
