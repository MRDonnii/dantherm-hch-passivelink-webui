    ...
    afterheat: <>
      ...
      <Card title={lang === "da" ? "Styr efter rumtemperatur" : "Follow the room temperature"} lead={lang === "da" ? "Varmere indblæsning når huset er koldt" : "Warmer supply air when the house is cold"} icon={Thermometer}>
        <div className="settings-grid">
          ...
          <label>{lang === "da" ? "Rumtemperatur fra" : "Room temperature from"}<select value={s(form.afterheat_room_source, "t5")} onChange={e => set("afterheat_room_source", e.target.value)}><option value="t5">{lang === "da" ? "Anlæggets T5-føler" : "The unit's T5 sensor"}</option><option value="ha_average">{lang === "da" ? "Gennemsnit af HA-rum" : "Average of HA rooms"}</option>{roomOptions(form.afterheat_room_source)}</select></label>
