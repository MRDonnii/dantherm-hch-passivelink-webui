    ...
    house: <>
      <Card title={t(SECTIONS[0].title)} lead={lang === "da" ? "Bygningsreglementet (BR18) kræver mindst 0,3 l/s pr. m² plus udsugning fra køkken og vådrum." : "The Danish building regulations (BR18) require at least 0.3 l/s per m² plus extract from kitchen and wet rooms."} icon={House}>
        <div className="settings-grid">
          <label className="check-row"><input type="checkbox" checked={sizing} onChange={check("sizing_enabled")}/> {lang === "da" ? "Styr grundtrin efter husets størrelse" : "Set the base level from the house size"}</label>
          <Help>{lang === "da" ? "Når den er slået til, bruger controlleren det beregnede grundtrin i stedet for Normaltrin, og nat, ferie og tør luft kan aldrig gå under det reducerede minimum. Manuel drift påvirkes ikke." : "When on, the controller uses the calculated base level instead of Normal level, and night, vacation and dry-air protection can never go below the reduced minimum. Manual mode is not affected."}</Help>
    ...
    afterheat: <>
      ...
      <Card title={lang === "da" ? "Styr efter rumtemperatur" : "Follow the room temperature"} lead={lang === "da" ? "Varmere indblæsning når huset er koldt" : "Warmer supply air when the house is cold"} icon={Thermometer}>
        <div className="settings-grid">
          ...
          <label>{lang === "da" ? "Rumtemperatur fra" : "Room temperature from"}<select value={s(form.afterheat_room_source, "t3")} onChange={e => set("afterheat_room_source", e.target.value)}><option value="t3">{lang === "da" ? "Udsugningsluft T3 (husets gennemsnit)" : "Extract air T3 (house average)"}</option><option value="t5">{lang === "da" ? "HRC2-rumføler T5 (upålidelig uden HCP4)" : "HRC2 room sensor T5 (unreliable without HCP4)"}</option><option value="ha_average">{lang === "da" ? "Gennemsnit af HA-rum" : "Average of HA rooms"}</option>{roomOptions(form.afterheat_room_source)}</select><Help>{lang === "da" ? "T3 måles altid af anlægget og svarer til husets gennemsnit. T5 sidder i HRC2-fjernbetjeningen og opdateres ikke, når Pi'en har erstattet HCP4." : "T3 is always measured by the unit and reflects the house average. T5 sits in the HRC2 remote and is not updated once the Pi has replaced HCP4."}</Help></label>
