...
class ControllerRuntime:
    ...
    def snapshot(self) -> dict[str, object]:
        ...
        result.update({
            ...
            "actual_supply_before_heater_temperature": t2,
            "actual_supply_before_heater_temperature_source": t2_source,
            "actual_supply_before_heater_temperature_age_seconds": round(t2_age, 1) if t2_age is not None else None,
            "actual_supply_before_heater_temperature_state": "fresh" if t2 is not None else ("stale" if t2_age is not None else "missing"),
            "actual_supply_air_temperature": t2ah,
            "actual_supply_air_temperature_source": t2ah_source,
            "actual_supply_air_temperature_age_seconds": round(t2ah_age, 1) if t2ah_age is not None else None,
            "actual_supply_air_temperature_state": "fresh" if t2ah is not None else ("stale" if t2ah_age is not None else "missing"),
            "actual_afterheat_frost_temperature": frost,
            "actual_afterheat_frost_temperature_source": frost_source,
            "actual_afterheat_frost_temperature_age_seconds": round(frost_age, 1) if frost_age is not None else None,
            "actual_afterheat_frost_temperature_state": "fresh" if frost is not None else ("stale" if frost_age is not None else "missing"),
